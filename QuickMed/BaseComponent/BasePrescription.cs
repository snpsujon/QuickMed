...
namespace QuickMed.BaseComponent
{
    public class BasePrescription : ComponentBase
    {
        ...
        public List<TblDurationNumber> durationNumber = new List<TblDurationNumber>();
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            adviceMasters = new();
            adviceMasters = await _adviceTemp.GetOnlyAdviceData();
            ...
            durationNumber = new();
            durationNumber = await App.Database.GetTableRowsAsync<TblDurationNumber>("TblDurationNumber");

            Instructions = new();
            Instructions = await App.Database.GetTableRowsAsync<TblInstruction>("TblInstruction");
            notesMasters = new();
            notesMasters = await _notes.GetAsync(); // Load the initial data
            ixMasters = new();
            ixMasters = await _ixTemp.GetAsync();
            ...
            // Extract the query parameter 'code' from the URL
            ...
            if (queryParams.ContainsKey("code"))
            {
                ...
                await GetPorPResult(Code, false);
                if (queryParams.ContainsKey("ispreview"))
                {
                    var Previrw = queryParams["ispreview"];
                    if (Previrw == "true")
                    {
                        await PreviewOnly(true, true);
                    }
                }
                //await PreviewOnly();
                //CallYourFunction();
        ...
        public async Task SaveOnly()
        {
            try
            {
                ...
                if (result.ValueKind != JsonValueKind.Undefined && result.ValueKind != JsonValueKind.Null)
                {
                    ...
                    if (result.TryGetProperty("ccTableData", out JsonElement ccTableDataArray))
                    {
                            ...
                            ccTableDataArray[0].TryGetProperty("column_2", out _))
                        {
                            ...
                            foreach (var cc in CCdataList)
                            {
                                ...
                                TblDurationNumber duration = new();
                                ...
                                if (!isDurationGuid)
                                {
                                    ...
                                    await _pres.SaveTblDurationNumberAsync(duration); // Create the new duration template
                                }
                    ...
                    await JS.InvokeVoidAsync("showAlert", "Success", "Prescription Saved Succussfully", "success", "swal-success");
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<dynamic> GetPorPResult(string input, bool isMobile)
        {
            if (isMobile)
            {
                TblPatient pdata = await _pres.GetPorPResult(input, isMobile);
                return pdata;
            }
            else //(!isMobile)
            {
        ...
        public async Task PreviewOnly(bool? IsHeader = true, bool? IsPrint = false)
        {
            try
            {
        ...
        public async Task SaveAndPreview()
        {
            try
            {
                await SaveOnly();
        ...
        public async Task SaveAndPreviewWithOutHeader()
        {
            try
            {
                await SaveOnly();
                await PreviewOnly(false, true);
            }
            catch (Exception)
            {
